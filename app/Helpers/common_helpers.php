...
if (! function_exists('current_package')) {
    function current_package() {
        ...
        if($subscription){
             ...
             $play = App\Models\PlayListing::where('user_id', auth()->user()->id)->count(); 
             $sleep = App\Models\SleepListing::where('user_id', auth()->user()->id)->count(); 
             $real_estate = App\Models\WorkListing::where('user_id', auth()->user()->id)->count();

             $totalListing = $beauty +  $car +  $play + $sleep + $real_estate;
