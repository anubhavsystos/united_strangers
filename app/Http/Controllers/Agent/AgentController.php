...
class AgentController extends Controller
{
    ...
    public function my_listings() 
    {
        ...
        $WorkListings = WorkListing::where('user_id', $userId)->get();
        $SleepListings = SleepListing::where('user_id', $userId)->get();
        $playListings = PlayListing::where('user_id', $userId)->get();
      
        // Merge the listings into a single collection
        $listings = $beautyListings
            ->concat($carListings)
            ->concat($WorkListings)
            ->concat($SleepListings)
            ->concat($playListings);
    ...
    public function listing_edit($id, $type, $tab='') {
        ...
        if($type == 'car'){
            ...
            $listing = SleepListing::where('id', $id);
        }elseif($type == 'work'){
            $listing = WorkListing::where('id', $id);
        }elseif($type == 'play'){
            $listing = PlayListing::where('id', $id);
        }
        $page_data['tab'] = $tab;
        $page_data['type'] = $type;
        $page_data['categories'] = Category::where('type', $type)->get();
    ...
    public function agent_ListingsFilter(Request $request)
{
    ...
    if ($request->has('type') && $request->type != 'choose') {
        if ($request->type == 'beauty') {
            ...
            $listings = $listings->merge(SleepListing::where('user_id', $userId)->where('visibility', $request->visibility ?? 'visible')->get());
            if (isset($request->visibility) && $request->visibility != 'all') {
                $listings = $listings->where('visibility', $request->visibility);
            }
        } elseif ($request->type == 'play') {
            $listings = $listings->merge(PlayListing::where('user_id', $userId)->where('visibility', $request->visibility ?? 'visible')->get());
            if (isset($request->visibility) && $request->visibility != 'all') {
                $listings = $listings->where('visibility', $request->visibility);
            }
        } elseif ($request->type == 'work') {
            $listings = $listings->merge(WorkListing::where('user_id', $userId)->where('visibility', $request->visibility ?? 'visible')->get());
            if (isset($request->visibility) && $request->visibility != 'all') {
                $listings = $listings->where('visibility', $request->visibility);
            }
