...
class OfferController extends Controller
{
    ...
    public function offer_edit($type, $id, $tab = ""){  
        if($type == 'work'){
            $page_data['offer'] =  $this->workoffer->where('id', $id)->first();
        }elseif($type == 'sleep'){
            $page_data['offer'] = $this->sleepoffer->where('id', $id)->first();
        }elseif($type == 'play'){
            $page_data['offer'] = $this->playoffer->where('id', $id)->first();
        }
        $page_data['categories'] = $this->category->where('type', $type)->get();
        $page_data['tab'] = $tab;
        $page_data['type'] = $type;
         
        return view('admin.offer.'.$type.'_edit', $page_data);
    }

    public function offer_update(Request $request, $type, $id){
        ...
        if($type == 'car'){
            ...
            $offer_image = json_decode(Caroffer::where('id', $id)->pluck('image')->toArray()[0])??[];

            if ($request->hasFile('offer_image')) {
                foreach ($request->file('offer_image') as $key => $image) {
                    $imageName = $key.'-'.time() . '.' . $image->getClientOriginalExtension();
                    $image->move(public_path('uploads/offer-images'), $imageName);
                    array_push($offer_image, $imageName);
                }
                $data['image'] = json_encode($offer_image);
            }else{
                $data['image'] = $offer_image;
            }
            
            Caroffer::where('id', $id)->update($data);
            Session::flash('success', get_phrase('offer Update successfully!'));
            if(isset($request->is_agent) && $request->is_agent == 1){
                return redirect('agent/my-offers');
            }else{
                return redirect('admin/offers/car');
            }
        }elseif($type == 'beauty'){
            ...
            $offer_image = json_decode(Beautyoffer::where('id', $id)->pluck('image')->toArray()[0])??[];
            ...
            Session::flash('success', get_phrase('offer Update successfully!'));
            Beautyoffer::where('id', $id)->update($data);

            if(isset($request->is_agent) && $request->is_agent == 1){
                return redirect('agent/my-offers');
            }else{
                return redirect('admin/offers/beauty');
            }
            ...
            $offer_image = json_decode($this->sleepoffer->where('id', $id)->pluck('image')->toArray()[0])??[];
            ...
            $this->sleepoffer->where('id', $id)->update($data);
            Session::flash('success', get_phrase('offer Update successfully!'));
            if(isset($request->is_agent) && $request->is_agent == 1){
                return redirect('agent/my-offers');
            }else{
                return redirect('admin/offers/sleep');
            }
        }elseif($type == 'work'){
            ...
            $offer_image = json_decode( $this->workoffer->where('id', $id)->pluck('image')->toArray()[0])??[];
            ...
            $offer_offer_floor_plan = json_decode( $this->workoffer->where('id', $id)->pluck('floor_plan')->toArray()[0])??[];

            if ($request->hasFile('offer_floor_plan')) {
                foreach ($request->file('offer_floor_plan') as $key => $image) {
                    ...
                    array_push($offer_offer_floor_plan, $floorImage);
                }
                $data['floor_plan'] = json_encode($offer_offer_floor_plan);
            }else{
                $data['floor_plan'] = json_encode($offer_offer_floor_plan);
            }

            Session::flash('success', get_phrase('offer Update successfully!'));
             $this->workoffer->where('id', $id)->update($data);
            if(isset($request->is_agent) && $request->is_agent == 1){
                return redirect('agent/my-offers');
            }else{
                return redirect('admin/offers/work');
            }
        }elseif($type == 'play'){
            ...
            $offer_image = json_decode($this->playoffer->where('id', $id)->pluck('image')->toArray()[0])??[];
            ...
            $this->playoffer->where('id', $id)->update($data);
            Session::flash('success', get_phrase('offer Update successfully!'));
            if(isset($request->is_agent) && $request->is_agent == 1){
                return redirect('agent/my-offers');
            }else{
                return redirect('admin/offers/play');
            }
        }
    }


    public function offer_image_delete($type, $id, $image){
        if($type == 'car'){
            $offer = Caroffer::where('id', $id);
        }elseif($type == 'beauty'){
            $offer = Beautyoffer::where('id', $id);
        }elseif($type == 'sleep'){
            $offer = $this->sleepoffer->where('id', $id);
        }elseif($type == 'work'){
            $offer =  $this->workoffer->where('id', $id);
        }elseif($type == 'play'){
            $offer = $this->playoffer->where('id', $id);
        }
        $imageToRemove = $image;
        $imageArray = json_decode($offer->first()->image);
        ...
        $offer->update(['image'=>$resultJson]);
        if(file_exists('public/uploads/offer-images/'.$image)){
            unlink('public/uploads/offer-images/'.$image);
        }
        return 1;
    }
    public function offer_floor_image_delete($type, $id, $image){
        if($type == 'car'){
            $offer = Caroffer::where('id', $id);
        }elseif($type == 'beauty'){
            $offer = Beautyoffer::where('id', $id);
        ...
        $imageToRemove = $image;
        $imageArray = json_decode($offer->first()->floor_plan);
        $key = array_search($imageToRemove, $imageArray);
        if ($key !== false) {
            unset($imageArray[$key]);
        }
        $imageArray = array_values($imageArray);
        $resultJson = json_encode($imageArray);
        $offer->update(['floor_plan'=>$resultJson]);
    ...
    public function offer_status($type, $id, $status){
        ...
        $offer->update(['visibility'=>$status]);
        Session::flash('success', get_phrase('offer Update successfully!'));
        return redirect()->back();
    }

    public function offer_delete($type, $id){
        if($type == 'car'){
            $offer = Caroffer::where('id', $id);
        }elseif($type == 'beauty'){
            $offer = Beautyoffer::where('id', $id);
