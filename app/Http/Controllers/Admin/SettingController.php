...
class SettingController extends Controller
{
    ...
    public function system_settings_update(Request $request) {
        ...
        if(System_setting::where('key', 'form_builder')->get()->count() > 0) {
    ...
    public function website_setting_update(Request $request)
{
    ...
    if ($request->type == 'sleep') {
        if ($request->hasFile('sleep')) {
            $existing_logo = get_frontend_settings('sleep');
            if ($existing_logo && file_exists(public_path('uploads/category_type/' . $existing_logo))) {
                ...
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move($upload_path, $logo_filename);
                FrontendSettings::where('key', 'sleep')->update(['value' => $logo_filename]);
                Session::flash('success', get_phrase('Sleep image updated successfully!'));
        } else {
            Session::flash('error', get_phrase('No file uploaded for sleep!'));
        }
    }
    ...
    if ($request->type == 'real_estate') {
        if ($request->hasFile('real_estate')) {
            ...
            Session::flash('error', get_phrase('No file uploaded for Work!'));
        }
    }
    if ($request->type == 'restaurent') {
        if ($request->hasFile('restaurent')) {
            ...
            if ($existing_logo && file_exists(public_path('uploads/category_type/' . $existing_logo))) {
                ...
                Session::flash('success', get_phrase('Play image updated successfully!'));
        } else {
            Session::flash('error', get_phrase('No file uploaded for Play!'));
        }
    }
...
public function CompanyLogo(){
 ...
 public function Deletecompanylogo($id)
 {
    ...
    public function homepage_setting_update(Request $request) {
        ...
        if ($request->type == 'CarBanner') {
            ...
            if ($request->hasFile('car_banner')) {
                if ($car_banner && file_exists(public_path('uploads/homepage/car/' . $car_banner))) {
                    unlink(public_path('uploads/homepage/beauty/' . $car_banner));
                }
        ...
        if ($request->type == 'SleepBanner') {
            $title = sanitize($request->input('sleep_banner_title'));
            $description = sanitize($request->input('sleep_banner_description'));
            $video_url = sanitize($request->input('sleep_video_url'));
            $homePageSetting = HomePageSetting::where('type', 'SleepBanner')->where('key', 'SleepBanner')->first();
            $sleep_banner_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $sleep_banner = $sleep_banner_data ? $sleep_banner_data->image : null;
            if ($request->hasFile('sleep_banner')) {
                if ($sleep_banner && file_exists(public_path('uploads/homepage/sleep/' . $sleep_banner))) {
                    unlink(public_path('uploads/homepage/sleep/' . $sleep_banner));
                }
                $file = $request->file('sleep_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/sleep'), $logo_filename);
            } else {
                $logo_filename = $sleep_banner;
            }
            $value = json_encode([
                'title' => $title,
                'description' => $description,
            ...
            Session::flash('success', get_phrase('Sleep Banner updated successfully!'));
        }
        if ($request->type == 'SleepBooking') {
            $title = sanitize($request->input('sleep_booking_title'));
            $homePageSetting = HomePageSetting::where('type', 'SleepBooking')->where('key', 'SleepBooking')->first();
            $sleep_booking_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $sleep_booking = $sleep_booking_data ? $sleep_booking_data->image : null;
            if ($request->hasFile('sleep_booking_image')) {
                if ($sleep_booking && file_exists(public_path('uploads/homepage/sleep/' . $sleep_booking))) {
                    unlink(public_path('uploads/homepage/sleep/' . $sleep_booking));
                }
                $file = $request->file('sleep_booking_image');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/sleep'), $logo_filename);
            } else {
                $logo_filename = $sleep_booking;
            }
            $value = json_encode([
                'title' => $title,
                'image' => $logo_filename
            ...
            Session::flash('success', get_phrase('Sleep Booking updated successfully!'));
        }
        if ($request->type == 'SleepExclusive') {
            $title = sanitize($request->input('sleep_exclusive_title'));
            $description = sanitize($request->input('sleep_deals_title'));
            $homePageSetting = HomePageSetting::where('type', 'SleepExclusive')->where('key', 'SleepExclusive')->first();
            $sleep_exclusive_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $sleep_exclusive = $sleep_exclusive_data ? $sleep_exclusive_data->image : null;
            if ($request->hasFile('exclusive_banner')) {
                if ($sleep_exclusive && file_exists(public_path('uploads/homepage/sleep/' . $sleep_exclusive))) {
                    unlink(public_path('uploads/homepage/sleep/' . $sleep_exclusive));
                }
                $file = $request->file('exclusive_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/sleep'), $logo_filename);
            } else {
                $logo_filename = $sleep_exclusive;
            }
            $value = json_encode([
                'title' => $title,
                'description' => $description,
            ...
            Session::flash('success', get_phrase('Sleep Exclusive updated successfully!'));
        }
        if ($request->type == 'SleepSize') {
            $title = sanitize($request->input('sleep_size_title'));
            $description = sanitize($request->input('sleep_size_discount'));
            $homePageSetting = HomePageSetting::where('type', 'SleepSize')->where('key', 'SleepSize')->first();
            $sleep_size_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $sleep_size = $sleep_size_data ? $sleep_size_data->image : null;
            if ($request->hasFile('size_banner')) {
                if ($sleep_size && file_exists(public_path('uploads/homepage/sleep/' . $sleep_size))) {
                    unlink(public_path('uploads/homepage/sleep/' . $sleep_size));
                }
                $file = $request->file('size_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/sleep'), $logo_filename);
            } else {
                $logo_filename = $sleep_size;
            }
            $value = json_encode([
                'title' => $title,
                'description' => $description,
            ...
            Session::flash('success', get_phrase('Sleep Size updated successfully!'));
        }
        ...
        if ($request->type == 'RealEstateBanner') {
            ...
            if ($request->hasFile('real_banner')) {
                if ($real_banner && file_exists(public_path('uploads/homepage/work/' . $real_banner))) {
                    unlink(public_path('uploads/homepage/work/' . $real_banner));
                }
                $file = $request->file('real_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/work'), $logo_filename);
            } else {
        ...
        if ($request->type == 'RealEstateDiscount') {
            ...
            if ($request->hasFile('real_discount_image')) {
                if ($real_discount && file_exists(public_path('uploads/homepage/work/' . $real_discount))) {
                    unlink(public_path('uploads/homepage/work/' . $real_discount));
                }
        ...
        // play 
        if ($request->type == 'PlayBanner') {
            $title = sanitize($request->input('play_banner_title'));
            $description = sanitize($request->input('play_banner_description'));
        
            $homePageSetting = HomePageSetting::where('type', 'PlayBanner')->where('key', 'PlayBanner')->first();
            $play_banner_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            
            $play_banner1 = $play_banner_data ? $play_banner_data->image1 : null;
            $play_banner2 = $play_banner_data ? $play_banner_data->image2 : null;
        
            // Handling first image upload
            if ($request->hasFile('play_banner1')) {
                if ($play_banner1 && file_exists(public_path('uploads/homepage/play/' . $play_banner1))) {
                    unlink(public_path('uploads/homepage/play/' . $play_banner1));
                }
                $file1 = $request->file('play_banner1');
                $logo_filename1 = time() . '_1_' . $file1->getClientOriginalName();
                $file1->move(public_path('uploads/homepage/play'), $logo_filename1);
            } else {
                $logo_filename1 = $play_banner1;
            }
        
            // Handling second image upload
            if ($request->hasFile('play_banner2')) {
                if ($play_banner2 && file_exists(public_path('uploads/homepage/play/' . $play_banner2))) {
                    unlink(public_path('uploads/homepage/play/' . $play_banner2));
                }
                $file2 = $request->file('play_banner2');
                $logo_filename2 = time() . '_2_' . $file2->getClientOriginalName();
                $file2->move(public_path('uploads/homepage/play'), $logo_filename2);
            } else {
                $logo_filename2 = $play_banner2;
            }
            ...
            Session::flash('success', get_phrase('Play Banner updated successfully!'));
        }
        if ($request->type == 'PlayExclusive') {
            $title = sanitize($request->input('play_exclusive_title'));
            $description = sanitize($request->input('play_deals_title'));
            $homePageSetting = HomePageSetting::where('type', 'PlayExclusive')->where('key', 'PlayExclusive')->first();
            $play_exclusive_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $play_exclusive = $play_exclusive_data ? $play_exclusive_data->image : null;
            if ($request->hasFile('play_exclusive_banner')) {
                if ($play_exclusive && file_exists(public_path('uploads/homepage/play/' . $play_exclusive))) {
                    unlink(public_path('uploads/homepage/play/' . $play_exclusive));
                }
                $file = $request->file('play_exclusive_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/play'), $logo_filename);
            } else {
                $logo_filename = $play_exclusive;
            }
            $value = json_encode([
                'title' => $title,
                'description' => $description,
            ...
            Session::flash('success', get_phrase('Play Exclusive updated successfully!'));
        }
        if ($request->type == 'PlayDiscount') {
            $title = sanitize($request->input('play_discount_title'));
            $description = sanitize($request->input('play_discount_description'));
            $homePageSetting = HomePageSetting::where('type', 'PlayDiscount')->where('key', 'PlayDiscount')->first();
            $play_discount_data = $homePageSetting ? json_decode($homePageSetting->value) : null;
            $play_discount = $play_discount_data ? $play_discount_data->image : null;
            if ($request->hasFile('play_discount_banner')) {
                if ($play_discount && file_exists(public_path('uploads/homepage/play/' . $play_discount))) {
                    unlink(public_path('uploads/homepage/play/' . $play_discount));
                }
                $file = $request->file('play_discount_banner');
                $logo_filename = time() . '_' . $file->getClientOriginalName();
                $file->move(public_path('uploads/homepage/play'), $logo_filename);
            } else {
                $logo_filename = $play_discount;
            }
            $value = json_encode([
                'title' => $title,
                'description' => $description,
            ...
            Session::flash('success', get_phrase('Play Discount updated successfully!'));
        }
