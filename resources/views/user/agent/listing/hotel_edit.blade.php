            ...
            <div class="col-lg-8 col-xl-9">
                ...
                <div class="ca-content-card p-4 position-relative">
                    <ul class="nav nav-pills cap-nav-pills mb-20px" id="pills-tab" role="tablist">
                        ...
                        <li class="nav-item" role="presentation">
                          <button class="nav-link {{$tab == 'room' ? 'active':''}}" id="pills-rooms-tab" data-bs-toggle="pill" data-bs-target="#pills-rooms" type="button" role="tab" aria-controls="pills-rooms" aria-selected="false">{{get_phrase('Rooms')}}</button>
                        </li>
                        <li class="nav-item" role="presentation">
                    ...
                    <form action="{{route('user.listing.update',['type'=>'sleep','id'=>$listing->id])}}" id="form-action" method="post" enctype="multipart/form-data">
                        @csrf
                    ...
                    <div class="tab-content" id="pills-tabContent">
                        ...
                        <div class="tab-pane fade show {{$tab == 'room' ? 'active':''}}" id="pills-rooms" role="tabpanel" aria-labelledby="pills-rooms-tab" tabindex="0">
                            <div class="d-flex align-items-center justify-content-between mb-2">
                                <h5 class="fs-16px title mb-3"> {{get_phrase('Add some Room')}} </h5>
                                <a href="javascript:void()" onclick="modal('modal-xl', '{{route('admin.add.listing.room',['prefix'=>'agent','id'=>$listing->id,'room_id'=>0, 'page'=>'add'])}}', '{{get_phrase('Add New Room')}}')" class="btn btn-primary fs-14px"> {{get_phrase('Add Room')}} </a>
                            </div>
                            @php
                                $rooms = App\Models\Room::where('listing_id', $listing->id)->where('user_id', user('id'))->get();
                            @endphp
                            <div class="row">
                                ...
                                <div class="col-sm-4"> 
                                    ...
                                    <label class="form-check-label w-100" onclick="room_select('{{$key}}')" for="flckDefault{{$key}}">
                                        <div class="card mb-3 room-checkbox">
                                            <div class="row g-0">
                                                <div class="col-md-4">
                                                    <img class="object-fit" src="{{ get_all_image('room-images/' . (json_decode($room->image)[0] ?? 'default.jpg')) }}" class="img-fluid rounded-start" alt="">
                                                </div>
                                                <div class="col-md-8 room-body eRoom fs-14px">

                                                    <div class="card-body py-0 px-2 h-100">
                                                        <p class="card-title mb-0 line-3"> {{$room->title}} </p>
                                                        <p class="card-text fs-12px"> {{currency($room->price)}} </p>
                                                        <div class="text-end position-absulate">
                                                            <a data-bs-toggle="tooltip" 
                                                            data-bs-title="{{  get_phrase('Edit') }}" href="javascript:void(0);" onclick="modal('modal-xl', '{{route('admin.add.listing.room',['prefix'=>'agent','id'=>$listing->id,'room_id'=>$room->id, 'page'=>'edit'])}}', '{{get_phrase('Update Room')}}')" class="p-1"> <i class="fas fa-edit"></i> </a>
                                                            <a data-bs-toggle="tooltip" 
                                                            data-bs-title="{{  get_phrase('delete') }}" href="javascript:void(0);" onclick="delete_modal('{{route('admin.delete.listing.room',['prefix'=>'agent','id'=>$room->id,'listing_id'=>$listing->id])}}')" class="p-1"> <i class="fas fa-trash-alt"></i> </a>
                                                        </div>
                                                    </div>
                        ...
                        <div class="tab-pane fade" id="shop" role="tabpanel" aria-labelledby="shop-tab">
                            <div class="d-flex align-items-center justify-content-between mb-5">
                                ...
                                <div>
                                    <a href="javascript:void(0);" onclick="modal('modal-md', '{{ route('agent.inventory.create', ['prefix' => 'agent', 'type' => 'sleep','listing_id' => $listing->id]) }}', '{{ get_phrase('Add Product') }}')" class="btn ol-btn-primary fs-14px"> {{ get_phrase('Add New Product') }} </a>
                                    <a href="javascript:void(0);" onclick="modal('modal-md', '{{ route('agent.inventory.category.create', ['prefix' => 'agent', 'type' => 'sleep', 'listing_id' => $listing->id]) }}', '{{ get_phrase('Add Category') }}')" class="btn ol-btn-primary fs-14px"> {{ get_phrase('Add Product Category') }} </a>
                                </div>
                            </div>
...
<script>
    ...
    $("#form-action-btn").on('click', function() {
        ...
        if(listing_size && listing_bath && listing_bed && listing_price && listing_title && listing_category && listing_country && listing_city && listing_address && listing_post_code && listing_latitude && listing_longitude && listing_visibility && listing_dimension){
