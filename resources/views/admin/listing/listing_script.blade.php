...
<script>
    $("#offer-type").on('change', function() {
        var type = $(this).val();
        var baseUrl = "{{ route('admin.offer.type') }}";
        var url = baseUrl + '?type=' + encodeURIComponent(type);
        $.ajax({
            url: url,
            type: 'GET',
            success: function(response) {
                ...
                category.html($('<option>', {
                    value: '',
                    text: "{{ get_phrase('Select offer on') }}"
                }));

                $.each(response, function(index, item) {
                    category.append($('<option>', {
                        value: item.id,
                        text: item.title // Assuming 'title' is used in the select
                    }));
                });
            },
