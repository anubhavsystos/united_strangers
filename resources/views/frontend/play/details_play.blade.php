    ...
    <div class="container">
        <div class="row row-28 mb-80px">
            ...
            {{-- Shop Addon --}}
                ...
                <!-- Amenities -->
                <div class="play-amenities-area mb-50px">
                ...
                <!-- Reviews -->
                  <div class="sleep-details-reviews  mb-50px">
                    <div class="review-title-button d-flex align-items-center justify-content-between flex-wrap">
                  
                           @php 
                            $totalReview = App\Models\Review::where('listing_id', $listing->id)->where('type', 'play')->where('reply_id', NULL)->get(); 
                           @endphp
                           <h2 class="title">{{count($totalReview)}} {{get_phrase('Reviews')}}</h2>
                   
                        @if(auth()->check()) 
                            @php 
                               $user_review_count = App\Models\Review::where('listing_id', $listing->id)->where('type', 'play')->where('user_id', auth()->user()->id)->whereNull('reply_id')->first(); 
                               @endphp
                    ...
                    <!-- Single comment wrapper -->
                        ...
                        @php 
                           $reviews = App\Models\Review::where('listing_id', $listing->id)->where('reply_id', NULL)->where('type', 'play')->get();
                        @endphp
                ...
                $ownReview = null;
                if (auth()->check()) {
                    $ownReview = App\Models\Review::where('listing_id', $listing->id)->where('type', 'play')
